...
selection and augmentation techniques of regression tests for security policy
...
Columns 2-4 show the lines of code, and the numbers of test cases and security test cases.
Security test cases refer to test cases that issue at least one request to the PDP.
Table~\ref{tab:subj} shows that our subjects include about 64 test cases and 43 security
test cases on average.
In particular, \CodeIn{VMS} is equipped with only 10 security tests (out of 52 test cases), which
...
Moreover, while \CodeIn{VMS} consists of 106 rules, \CodeIn{VMS} is equipped with only 10 security test cases
...
\begin{itemize}
	...
	\item RQ1: How high percentage of test cases (from an existing test suite) is reduced by our test selection techniques? This question helps to show that our techniques can 
...
%	\item RQ2: Do our protest selections are safe? This question helps to show that our techniques select all tests, which shows only and all test cases to reveal changed behaviors in access control policies.
	...
	\item RQ3: How high is the additional policy coverage ratio that is achieved by our test augmentation technique?  This question helps to show that our technique can generate augmented test cases to achieve additional coverage of changed policy behaviors.
...
%technique. For testing access control policies, a test is synonymous with request
	...
	policy, the test reduction percentage is the number of selected test cases for regression testing divided by the number of security test cases.
		\item \textit{The number of rules impacted by policy changes ($\# CT$).}  Given a policy and its modified
	policy, this metric shows the number of rules impacted by policy changes.
	\item \textit{The coverage of rules impacted by policy changes ($\# COV$).}  Given a policy and its modified
	policy, this metric shows the number of
	impacted rules covered with existing test cases.
	\item \textit{The percentage of coverage of rules impacted by policy changes ($\% COV$).}  Given a policy and its modified
	policy, the percentage of coverage of rules impacted by policy changes is $\frac{\# COV}{\# CT}$.
...
%	policy, the changed policy behavior coverage percentage is the changed policy behavior coverage count divided by the total number of impacted rules.
	...
	To assess effectiveness of test reduction (RQ1), we use a test reduction percentage metric with additional $\# CT$, $\# COV$, $\% COV$ metrics.
	To assess efficiency of test selection (RQ2), we use elapsed time.
	To assess effectiveness of test augmentation (RQ3), we use a augmented test case count metric.
...
\begin{table}[thbp]
  ...
  \caption{Test case type ratios of augmented test cases for our subjects}
    \begin{tabular}{|l|r|r|r|r|}
		\hline
    Subjects & \% M-Sub & \% M-Cond & \% M-SubCond \\\hline\hline
    LMS   & 21.83 & 4.93  & 11.27  \\\hline
    VMS   & 3.41  & 0.38  & 1.89  \\\hline
    ASMS  & 16.30 & 9.63  & 7.41 \\\hline\hline
    Average & 13.85 & 4.98  & 6.86  \\\hline
...
For those requests which are not classified to  ``M-Sub'', ``M-Cond'', or ``M-SubCond'',
the requests may require the developers to create new test cases issuing requests
...
we achieve our goal by modifying such variables. 
%However, for `M-Others'', we create
%new test cases.
...
cases. If we aim to achieve additional coverage, we could generate the augmented test cases by scratch since we could not find similar
test cases to modify in context of our subjects. For such a task, we may need to find all relevant methods with required information to cover a certain rule, which is not trivial. Besides some of those rules may not be covered by any PEP code similar to dead code that is not reachable. 
%Total of each row is 100\%, which indicates that we
