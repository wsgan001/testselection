...
behaviors (reflected by rules impacted by policy changes) thus could be applied to other policy models beyond
an OrBAC policy model~\cite{kalam03:orBac}.
The reason is because, while different policy models may
...
policies based on various policy models (such as RBAC~\cite{anderson04:rbacxacml,ferraiolo01:proposed}
and OrBAC).
...
Additional regression model to modify various policy attributes could simulate various policy modifications.
While \CodeIn{RMR} and \CodeIn{RDC} may not feed specific changes
of attribute elements in a rule, \CodeIn{RA} could be more flexible
to simulate adding/changing any rules that the developers would like to modify
in practice. For changing specific attribute elements in rules $rs$, we may replace $rs$ by removing
$rs$ and add new rules $rs'$ reflected by the specific changes at the same location.
Our regression model can simulate any kinds of policy changes using
