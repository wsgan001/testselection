...
%. From a crash trace A°ÊB°ÊC°ÊD shown in Figure 
%3
...
\CodeIn{M-Others} denotes an argumentation type, which
...
application code to issue requests $NR_{req}$ cover $NR$.
...
While \CodeIn{M-Sub}, \CodeIn{M-Cond}, and \CodeIn{M-SubCond} require developers modify existing
...
we achieve our goal by modifying such variables. However, for \CodeIn{M-Others}, the augmentation technique require the
