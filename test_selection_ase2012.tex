...
\maketitle
\begin{abstract}
As security requirements of software often change, developers may modify security policies such as access control policies (policies in short) according to evolving requirements. 
...
that may reveal regression faults caused by policy changes. In addition, since existing test cases may not evaluate (i.e., cover) all the rules impacted by policy changes, we develop a test augmentation technique by generating additional test cases to cover the rules that are not covered by existing test cases.
